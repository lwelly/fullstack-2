<template>
  <div class="rec-detail-page">
    ...
    <div v-else-if="error" class="error-state">
      <v-icon size="80" color="grey-lighten-1">mdi-file-document-alert-outline</v-icon>
    ...
    <v-row v-else-if="rec" class="main-content" gutter="16">
      ...
      <v-col cols="12" md="8">
        ...
        <!-- Bannière de statut -->
        <div class="status-banner" :style="{ background: statusBg(rec.status), borderColor: statusColor(rec.status) }">
          ...
          <div class="status-banner-date" v-if="rec.resolved_at || rec.updated_at">
        ...
        <v-card class="progress-card mb-4" elevation="0" border>
          <v-card-title class="progress-title">
          ...
          <div class="progress-steps">
            ...
            >
              ...
              <div class="step-connector" v-if="i < progressSteps.length - 1" />
        ...
        <v-card class="detail-card mb-4" elevation="0" border>
        ...
        <v-row dense class="mb-4">
          <v-col cols="12" sm="6">
            <v-card elevation="0" border class="module-card h-100">
              <v-card-text>
          ...
          <v-col cols="12" sm="6">
            <v-card elevation="0" border class="semestre-card h-100">
              <v-card-text>
                <div class="module-header">
                  ...
                  <div>
                    <p class="module-code">{{ rec.semestre?.code }}</p>
        ...
        <v-card
          ...
          class="admin-response-card mb-4"
          elevation="0"
          border
        ...
        >
          <v-card-title
            ...
            :style="{ background: rec.status === 'rejected' ? '#FEF2F2' : '#F0FDF4' }"
          >
            <v-icon
              :color="rec.status === 'rejected' ? 'error' : 'success'"
              class="mr-2"
            >
          ...
          <v-card-text class="admin-response-body">
            {{ rec.admin_response }}
          </v-card-text>
        </v-card>
        ...
        <!-- Réunion planifiée -->
        <v-card
          v-if="rec.meeting?.scheduled_at"
          class="meeting-card mb-4"
          elevation="0"
          border
        >
          ...
          <v-card-text>
            <v-row dense>
              ...
              <v-col cols="12" sm="6" v-if="rec.meeting.location">
                <div class="info-row">
                  <span class="info-label">Lieu</span>
                  <span class="info-value">{{ rec.meeting.location }}</span>
                </div>
              </v-col>
              <v-col cols="12" v-if="rec.meeting.notes">
                <div class="info-row flex-col">
        ...
        >
          ...
          <v-card-text>
            ...
            >
              <v-icon :color="attIconColor(att.mime_type)" size="32">
                {{ attIcon(att.mime_type) }}
              </v-icon>
              <div class="att-info">
        ...
        <v-card class="history-card mb-4" elevation="0" border>
          <v-card-title class="section-title">
          ...
          <v-card-text>
            <div v-if="!history.length" class="no-history">
              <v-icon color="grey-lighten-2" size="36">mdi-timeline-outline</v-icon>
            ...
            >
              ...
              >
                <div class="timeline-item">
                  <div class="timeline-header">
                    <div class="timeline-statuses">
                      <v-chip
                        ...
                        size="x-small"
                        variant="tonal"
                        :color="statusColor(h.old_status)"
                      >
                  ...
                  <p class="timeline-comment" v-if="h.comment">{{ h.comment }}</p>
      ...
      <v-col cols="12" md="4">
        ...
        <!-- Infos rapides -->
        <v-card class="quick-info-card mb-4" elevation="0" border>
          <v-card-title class="section-title">
          ...
          <v-list density="compact" class="quick-list">
            <v-list-item>
              <template #prepend>
                <v-icon size="18" color="grey">mdi-tag-outline</v-icon>
              </template>
              <v-list-item-title class="quick-label">Référence</v-list-item-title>
              <template #append>
                <span class="quick-value">{{ rec.reference_number }}</span>
              </template>
            </v-list-item>

            <v-list-item>
              <template #prepend>
                <v-icon size="18" color="grey">mdi-format-list-bulleted-type</v-icon>
              </template>
              <v-list-item-title class="quick-label">Type</v-list-item-title>
              <template #append>
                <v-chip size="x-small" :color="typeColor(rec.type)" variant="tonal">
                  {{ typeLabel(rec.type) }}
                </v-chip>
              </template>
            </v-list-item>
            ...
            <v-list-item>
              <template #prepend>
                <v-icon size="18" color="grey">mdi-calendar-plus</v-icon>
              </template>
              <v-list-item-title class="quick-label">Soumis le</v-list-item-title>
              <template #append>
                <span class="quick-value">{{ fDate(rec.created_at) }}</span>
              </template>
            </v-list-item>
            ...
            <v-list-item v-if="rec.resolved_at">
              ...
              <template #append>
                <span class="quick-value">{{ fDate(rec.resolved_at) }}</span>
              </template>
            </v-list-item>

            <v-list-item>
              <template #prepend>
                <v-icon size="18" color="grey">mdi-timer-sand</v-icon>
              </template>
              <v-list-item-title class="quick-label">Délai de traitement</v-list-item-title>
              <template #append>
                <span class="quick-value">{{ delayLabel }}</span>
              </template>
            </v-list-item>

            <v-list-item v-if="rec.academic_year">
              <template #prepend>
                <v-icon size="18" color="grey">mdi-school</v-icon>
              </template>
              <v-list-item-title class="quick-label">Année univ.</v-list-item-title>
              <template #append>
                <span class="quick-value">{{ rec.academic_year }}</span>
              </template>
            </v-list-item>
          </v-list>
        </v-card>
        ...
        <!-- Escalade -->
        <v-card
          v-if="rec.is_escalated"
          class="escalade-card mb-4"
          elevation="0"
          border
        >
          <v-card-title class="section-title escalade-title">
            <v-icon color="orange" class="mr-2">mdi-arrow-up-bold-circle</v-icon>
            Réclamation escaladée
          </v-card-title>
          <v-card-text>
            <p v-if="rec.escalation_reason" class="escalade-reason">
    ...
    <v-dialog v-model="confirmCancelDialog" max-width="460" persistent>
      <v-card class="dialog-card">
        ...
        <v-card-actions class="dialog-actions">
          <v-btn variant="text" @click="confirmCancelDialog = false" :disabled="cancelling">
            Conserver
          </v-btn>
          <v-btn
            color="error"
...
const STATUS_MAP = {
  submitted: {
    ...
    bg    : '#EFF6FF',
    desc  : 'Votre réclamation a été soumise et est en attente de réception.',
  },
  received: {
    label : 'Reçu',
    color : '#0891B2',
    ...
    bg    : '#ECFEFF',
    desc  : 'Votre réclamation a été reçue par l\'administration.',
  },
  in_review: {
    label : 'En cours',
    color : '#D97706',
    ...
    bg    : '#FFFBEB',
    desc  : 'Votre réclamation est en cours d\'examen.',
  },
  escalated: {
    label : 'Escaladé',
    color : '#EA580C',
    ...
    bg    : '#FFF7ED',
    desc  : 'Votre réclamation a été transmise à un responsable supérieur.',
  },
  resolved: {
    label : 'Résolu',
    color : '#059669',
    ...
    bg    : '#F0FDF4',
    desc  : 'Votre réclamation a été traitée avec succès.',
  },
  rejected: {
    label : 'Rejeté',
    color : '#DC2626',
    ...
    bg    : '#FFF5F5',
    desc  : 'Votre réclamation a été rejetée par l\'administration.',
  },
}
...
function statusBg(s)    { return STATUS_MAP[s]?.bg    ?? '#F8FAFC' }
...
const progressSteps = computed(() => {
  ...
  const idx     = current === 'rejected'
    ? STEP_ORDER.indexOf('in_review')
...
const delayLabel = computed(() => {
  ...
  const end   = rec.value.resolved_at
    ? new Date(rec.value.resolved_at)
    : new Date()
...
const canCancel = computed(() =>
  rec.value && ['submitted', 'received'].includes(rec.value.status)
)
...
async function load() {
  ...
  try {
    ...
    // Supporter { success, data: {...} } OU directement l'objet
    ...
    console.error('[ReclamationDetail] load error :', err)
    console.error('[ReclamationDetail] response   :', err.response?.data)

    const status = err.response?.status
    const msg    = err.response?.data?.message ?? err.response?.data?.error

    if (status === 404) {
      error.value = msg ?? 'Réclamation introuvable.'
    } else if (status === 403) {
      error.value = 'Accès non autorisé à cette réclamation.'
    } else if (status === 401) {
      router.push('/login')
    } else {
      error.value = msg ?? 'Impossible de charger la réclamation.'
    }
...
async function cancelReclamation() {
  ...
  try {
    await api.delete(`/student/reclamations/${rec.value.id}`, {
      data: { reason: cancelReason.value || undefined },
    })
...
.status-banner-text strong { font-size: 1rem; color: #1E293B; }
.status-banner-text span   { font-size: 0.83rem; color: #64748B; }
.status-banner-date {
  display: flex;
  align-items: center;
  gap: 4px;
...
/* ── Barre de progression ────────────────────────────────────────── */
.progress-card { border-radius: 12px !important; }
.progress-title {
...
.step-active .step-circle { background: #2563EB; border-color: #2563EB; box-shadow: 0 0 0 4px #DBEAFE; }
.step-rejected .step-circle { background: #DC2626; border-color: #DC2626; box-shadow: 0 0 0 4px #FEE2E2; }
...
.step-done ~ .progress-step .step-connector { background: #059669; }
.step-label {
  font-size: 0.72rem;
  font-weight: 500;
  color: #94A3B8;
...
.quick-label { font-size: 0.82rem !important; color: #64748B; }
.quick-value { font-size: 0.82rem; font-weight: 600; color: #334155; }

.escalade-title { color: #EA580C !important; }
.escalade-reason {
  font-size: 0.88rem;
  color: #475569;
  line-height: 1.6;
...
.dialog-body  { padding: 16px 24px !important; color: #334155; font-size: 0.9rem; }
.dialog-warning {
  color: #DC2626;
  font-size: 0.83rem;
  font-weight: 500;
